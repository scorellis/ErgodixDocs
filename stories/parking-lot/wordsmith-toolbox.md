# Wordsmith Toolbox — foundational rhetoric reference Skill
...
A single foundational reference is more valuable than per-tool inline definitions; the reference is consulted by tools, not directly by humans (humans get *output* from the tools, not raw rhetoric lookups); a TOML/YAML rubric is rich enough to encode each device (name, definition, exemplars, classifier hints) without prose; the umbrella "Wordsmith Toolbox" name keeps the framing neutral and craft-oriented — fits the methodology's directive that "Eloquence is intentionality" — and avoids religious/cultural friction in education-mode and commercial deploys.
...
- [ ] Cross-reference: [plot-planner](plot-planner.md) (Wordsmith Toolbox is a dependency of `writing-score`, `PC-placator`, and any future stylistic-feedback tool).
